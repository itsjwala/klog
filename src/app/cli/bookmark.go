...
type Bookmark struct {
	List  BookmarkList  `cmd name:"list" group:"Bookmark" help:"Show all available bookmarks"`
...
func (args *BookmarkList) Run(ctx app.Context) error {
	bs, err := ctx.Bookmarks()
	if err != nil {
		return err
	}
...
func (args *BookmarkEdit) Run(ctx app.Context) error {
	b, appErr := ctx.Bookmarks()
	if appErr != nil {
		return appErr
	}
	return ctx.OpenInEditor(b.GetDefault().Target.Path)
}
